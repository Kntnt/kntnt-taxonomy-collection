 ...
 * Description:       Provides the `collection` taxonomy used to to create an archive of posts that are part of a series, theme, or similar.
 * Version:           1.0.0
...
class Taxonomy {
    ...
    public function run() {
        ...
        register_taxonomy( 'collection', null, $this->taxonomy() );

        $post_types = apply_filters( 'kntnt-taxonomy-collection-objects', [ 'post' ] );
        foreach ( $post_types as $post_type ) {
            register_taxonomy_for_object_type( 'collection', $post_type );
        }
